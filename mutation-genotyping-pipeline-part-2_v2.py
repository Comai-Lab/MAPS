...
parser.add_option("-f", "--file", dest="f", default="sys.argv[1]", help="Input maps part 1 result file file.")
parser.add_option("-o", "--out", dest="o", default="sys.argv[2]", help="Output file name.")
parser.add_option("--minCov", "-c", dest="mincov", type = "int", default=1, help="Minimum position coverage")
parser.add_option("--maxCov", "-C", dest="mamaxcov", type = "int", default=100000000000000000000, help="Maximum mutant allele coverage")
...
head = f.readline()
head = head.replace('uifg-100-18-50', 'uifg-100-18-cut50')
header = head.split('\t')
newh = header[:4]+ ["#Libs"]
...
for l in f:
   ...
   tlibs = list(splitter(xline[16:],2))
   libs = []
   for item in tlibs:
      temp = item[:]
      if '.-' in temp[0]:
      ...
      for lib in libs:
         ...
         if mode == 'm':
            ...
            if lib[0] =='.':
               ...
               if base != common:
                  if int(lib[1]) >= minhom and int(lib[1]) <= opt.mamaxcov:
                     results.append(base)
                  else:
                     results.append('.')
                     totcovt -= int(lib[1])
               ...
               if s1base == common:
                  if s2cov >= minhet and s2per >= minhetper and s2cov <= opt.mamaxcov:
                     results.append(s1base+'-'+s2base)
                  else:
                     results.append('.')
                     totcovt -= int(lib[1])
               elif s2base == common:
                  if s1cov >= minhet and s1per >= minhetper and s1cov <= opt.mamaxcov:
